...
- **Star Force calculator** — a second top-level tab (**Planner | Star Force**) that prices taking one item from its current star to a target. Enter the item level (with 150/160/200/250 quick pills) and a current → target star range, then toggle **Star Catch**, **Safeguard** (15–17★, +200% cost, no booms), the v.269 **Enhancement Modes 1–4** (higher modes boom less but cost more; modes 2–4 also lower success on 18–21★), an **MVP tier** discount, and the two current GMS events — **Shining Star Force** (30% off cost + 30% fewer booms under 21★) and **1+1 Star Force** (+1★ per success under 11★, caps at 12★), independently toggleable and stackable. Results show the exact expected cost and boom count (closed-form math over the official v.269 rates, including boom-checkpoint re-climbs), expected attempts, a seeded-simulation **median** and **unlucky (top 10%)** run, and a per-star enhancement table (success/boom odds, cost per attempt, expected cost and booms per step). Inputs start empty and nothing is persisted. Rates and costs follow the GMS v.264+ 30★ tables (Enhancement Mode arrived in v.269); the per-mode tables and boom checkpoints are community-sourced.
