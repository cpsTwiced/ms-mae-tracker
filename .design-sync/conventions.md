...
Maplet is a cozy-dark MapleStory planner. Dark is the only look: warm charcoal surfaces, one sage-teal accent, generous rounding, compact density.

## Setup (required)

Wrap every app in `MapletProvider` — it is Mantine's provider preconfigured with the Maplet theme (sage accent, warm `dark` palette, `defaultRadius: 'lg'`, dark color scheme). Without it every component renders unstyled/light.
...
- Spacing: `var(--mantine-spacing-xs|sm|md|lg|xl)`; radius: `var(--mantine-radius-md|lg)`; font: `var(--mantine-font-family)`
